<?php 
    ...
    if(isset($_POST["submit-search"]) && isset($_POST['csrf_token']) && $_POST['csrf_token'] === $_SESSION['csrf_token']){
        ...
        $result1 = $conn->query("SELECT orders.order_id, order_items.order_item_id, products.product_name, order_items.quantity, order_items.price FROM orders JOIN order_items ON orders.order_id = order_items.order_id JOIN products ON products.product_id = order_items.product_id WHERE order_items.order_item_id like '%$query%'");
    }
    ...
    if(isset($_GET["orderId"])){
        ...
        $result1 = $conn->query("SELECT orders.order_id, order_items.order_item_id, products.product_name, order_items.quantity, order_items.price FROM orders JOIN order_items ON orders.order_id = order_items.order_id JOIN products ON products.product_id = order_items.product_id WHERE orders.order_id = $order_id ");
    }
?>
