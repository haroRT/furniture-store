...
// Bắt đầu session
    ...
    if(isset($_POST["submit-delete"]) && isset($_POST['csrf_token']) && $_POST['csrf_token'] === $_SESSION['csrf_token']){
        ...
        $result = $conn->query("DELETE FROM orders WHERE status = 'Chờ xác nhận' AND order_id=$order_id");
        if($result){
            $message = "thành công";
        }
        else{
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-4">Số lượng sản phẩm</th>
                            <th scope="col" class="px-4 py-3">Giá trị đơn hàng</th>
                        ...
                        <?php
                            while($row = $result1->fetch_assoc()){
                                echo '<tr class="border-b dark:border-gray-700">
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white"><a href="http://localhost/webphp/pages/order-items/order-items.php?orderId='.$row["order_id"].'">'.$row["order_id"].'</a></th>
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">'.$row["total_products"].'</th>
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">'.number_format($row["total_value"], 0, '', '.').'</th>
                                        <td class="px-4 py-3 max-w-[12rem] truncate">'.$row["status"].'</td>
                                        <td class="px-4 py-3 flex items-center justify-end">
                                            <button id="button" data-dropdown-toggle="'.$row["order_id"].'" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                                <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                                </svg>
                                            </button>
                                            <div id="'.$row["order_id"].'" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
...
<div id="deleteModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <form method="post" >     
                <p class="mb-4 text-gray-500 dark:text-gray-300">Bạn chắc chắn muốn xóa đơn hàng này?</p>
                <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
                <input type="text" name="order_id" id="delete-order_id" class="hidden bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-1/2 p-2 m-4 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="">
...
<script>
    ...
    editButtons.forEach(button => {
        button.addEventListener('click', function () {
            ...
            document.getElementById('update-order_id').value = categoryId;
        });
    });
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function () {
            const categoryId = this.closest('tr').querySelector('[data-dropdown-toggle]').getAttribute('data-dropdown-toggle');
            document.getElementById('delete-order_id').value = categoryId;
