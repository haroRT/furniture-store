...
<?php 
    ...
    if(isset($_POST["submit-search"]) && isset($_POST['csrf_token']) && $_POST['csrf_token'] === $_SESSION['csrf_token']){
        ...
        $result1 = $conn->query("SELECT orders.order_id, order_items.order_item_id, products.product_name, order_items.quantity, order_items.price FROM orders JOIN order_items ON orders.order_id = order_items.order_id JOIN products ON products.product_id = order_items.product_id WHERE orders.user_id = $user_id AND order_items.order_item_id like '%$query%'");
    }
    if(isset($_POST["submit-update"]) && isset($_POST['csrf_token']) && $_POST['csrf_token'] === $_SESSION['csrf_token']){
        $order_id = $_GET["orderId"];
        $order_item_id = $_POST["order_item_id"];
        $order_item_quantity = htmlspecialchars($_POST["order_item_quantity"], ENT_QUOTES, 'UTF-8');
        $resultCheck = $conn->query("SELECT orders.status FROM orders JOIN order_items ON orders.order_id = order_items.order_id WHERE orders.order_id = $order_id");
        $rowResultCheck = $resultCheck->fetch_assoc();
        if($rowResultCheck["status"] == "Chờ xác nhận"){
            $result = $conn->query("UPDATE order_items SET quantity = $order_item_quantity WHERE order_item_id = $order_item_id");
            if($result){
                $message = "thành công";
            }
            else{
    ...
    if(isset($_POST["submit-delete"]) && isset($_POST['csrf_token']) && $_POST['csrf_token'] === $_SESSION['csrf_token']){
        $order_item_id = $_POST["order_item_id"];
        $resultCheck = $conn->query("SELECT orders.status FROM orders JOIN order_items ON orders.order_id = order_items.order_id WHERE order_item_id = $order_item_id");
        $rowResultCheck = $resultCheck->fetch_assoc();
        if($rowResultCheck["status"] == "Chờ xác nhận"){
            $result = $conn->query("DELETE FROM order_items WHERE order_item_id=$order_item_id");
            if($result){
                $message = "thành công";
            }
            else{
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            <th scope="col" class="px-4 py-4">Id đơn hàng</th>
                            <th scope="col" class="px-4 py-4">Id chi tiết</th>
                            <th scope="col" class="px-4 py-3">Tên mặt hàng </th>
                        ...
                        <?php
                            while($row = $result1->fetch_assoc()){
                                echo '<tr class="border-b dark:border-gray-700">
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">'.$row["order_id"].'</th>
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">'.$row["order_item_id"].'</th>
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">' .$row["product_name"].'</th>
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">' .$row["quantity"].'</th>
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">' .$row["price"].'</th>
                                        <td class="px-4 py-3 flex items-center justify-end">
                                            <button id="button" data-dropdown-toggle="'.$row["order_item_id"].'" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                                <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                                </svg>
                                            </button>
                                            <div id="'.$row["order_item_id"].'" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
            ...
            <form method ="post">
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div class="hidden">
                        ...
                        <input type="text" name="order_item_id" id="update-category-id" value="" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="">
                    </div>
                    <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
                    <div>
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Số lượng sản phẩm</label>
                        <input type="number" name="order_item_quantity" min="1" value="1" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="">
                    </div>
                </div>
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <form method="post" >    
                ...
                <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
                <input type="text" name="order_item_id" id="delete-category-id" class="hidden bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-1/2 p-2 m-4 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="">
